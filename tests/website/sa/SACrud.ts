...
export const checkSA = {
    using: (SAInfo: DataTable) => {
        return Task.where(`#actor check fields`,
            Ensure.eventually(Text.of(SA.lookupInputFieldSingleValue('Primary Project')), includes(SAInfo.rowsHash().PrimaryProject)),
            Ensure.eventually(Text.of(SA.lookupInputFieldSingleValue('Primary Contract')), includes(SAInfo.rowsHash().PrimaryContract)),
