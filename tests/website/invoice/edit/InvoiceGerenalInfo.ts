...
import { COOKIE_CONTRACT_NO, COOKIE_INVOICE_NO, COOKIE_PURCHASE_ORDER_NO, INVOICE_PAYMENT, MANAGE_INVOICE } from '../../common/statics';
...
export const checkInvoiceGeneralInfo = {
    using: (invoiceInfo: Record<string, string | Question<any>>) =>
        Task.where(`#actor check invoice general information`,
            ...
            Check.whether(
                invoice.readOnlyLookupFieldBox('Purchase Order'), isVisible()
            ).andIfSo(
                Ensure.eventually(Text.of(invoice.readOnlyLookupFieldValue('Purchase Order')), includes(Cookie.called(COOKIE_PURCHASE_ORDER_NO).value())),
            ),
            ...
            invoice.checkTextInputFieldValue('Invoice No.', Cookie.called(COOKIE_INVOICE_NO).value(), SUCCEEDED),
            invoice.checkDropdownInputFieldValue('Invoice Status', invoiceInfo.InvoiceStatus, SUCCEEDED),
            ...
            invoice.checkTextInputFieldValue('Billing Address', invoiceInfo.BillingAddress, SUCCEEDED),
...
export const checkPaidInvoice = {
    using: (invoiceStatus:string) =>
        Task.where(`#actor check invoice general information`,
            invoice.checkDropdownInputFieldValue('Invoice Status', invoiceStatus, SUCCEEDED),
            checkButtonVisible.using(SAVE, FAILED),
            checkButtonVisible.using(DELETE, FAILED),
        )
