...
import { COOKIE_SCENARIO_NAME } from '../common/statics';
...
export const addScenario = {
    using: (scenarioInfo: DataTable) => {
        CS.timestamp = formatted_now
        return Task.where(`#actor Input all fields and save `,
            CS.selectDropdownItem('Cycle', scenarioInfo.rowsHash().Cycle),
            Wait.for(Duration.ofSeconds(3)),
            CS.fillTextInputField('Scenario Name', scenarioInfo.rowsHash().ScenarioName + CS.timestamp),
            CS.setCookie(COOKIE_SCENARIO_NAME, scenarioInfo.rowsHash().ScenarioName + CS.timestamp),
