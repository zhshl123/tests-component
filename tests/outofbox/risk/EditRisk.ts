...
export class EditRisk extends EditFromFields {
    ...
    selectItemInlookupPopup = (fieldName: string, itemName: string | Question<any>, popupFieldName: string) => {
        ...
        return Task.where(`#actor searchs single item : ${itemName} in ${fieldName} lookup popup`,
            // 先检查输入框中是否已有值
            Click.on(this.ralationshipAttributeLookupFieldClearIcon(fieldName)),
    ...
    /************************** Comment List ************************************** */
    commentSectionFrame = () =>
        PageElement.located(By.id(`ctl00_body_CommentsFieldrisks_iframeComment`))
            .describedAs('comment section')

    addCommentIcon = () =>
        PageElement.located(By.id(`commentControl_gv_ctl01_GridView_lnkNew`))
            .describedAs('add comment icon')

    addCommentPanel = () =>
        PageElement.located(By.css('#commentControl iframe'))
            .describedAs('add comment panel')
    addCommentPanelSaveButton = () =>
        PageElement.located(By.id('ctl00_phCustomButtons_btnSave'))
            .describedAs('add comment panel Save button')

    addCommentPanelCommentInputField = () =>
        PageElement.located(By.id('ctl00_body_Comment_txtText'))
            .describedAs('add comment panel Comment text input field')

    commentListTable = () =>
        PageElement.located(By.id('commentControl_gv'))
            .describedAs('comment list table')

    commentListTableCell = (comment: string) =>
        PageElement.located(By.cssContainingText('td', comment))
...
export const riskWorkflow = new Workflow(riskMap)