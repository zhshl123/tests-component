...
export class SearchFromFields {
    ...
    */
    ...
    selectLookupDropdownItem = (fieldName: string, itemName: string) => {
        return Task.where(`#actor selects dropdown item '${itemName}' in lookup field '${fieldName}'`,
            Click.on(this.inputFieldClearIcon(fieldName)),
            // 点击输入框
            Click.on(this.lookupInputField(fieldName)),
            ...
            Click.on(this.fieldLabel(fieldName))
    ...
    searchItemAndEdit = (pageName: string, fieldName: string, itemName: string | Question<any>) => {
        ...
        return Task.where(`#actor check search item: ${itemName} and forward to edit page`,
    ...
    */
    ...
    fieldLabel = (fieldName: string) =>
        PageElement.located(By.id('ctl00_body_A0_' + this.entityMap.get(fieldName) + '_FL'))
