...
export class EditFromFields {
    ...
    */
     ...
     * @returns 
     */
    checkSelectedSingleCheckbox = (fieldName: string, nth: string) => {
        return Task.where(`#actor check selected ${nth} single checkbox ${fieldName} `,
            Ensure.eventually(Attribute.called('checked').of(this.radioButtonInput(fieldName, nth)), equals('checked'))
        )
    }
    ...
    */
    ...
    selectNAICSItemInlookupPopup = (fieldName: string, itemName: string | Question<any>, popupFieldName: string) => {
        Task.where(`#actor searchs single item : ${itemName} in ${fieldName} lookup popup`,
