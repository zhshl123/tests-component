...
exports.checkCOGeneralInfo = {
    using: (changeOrderInfo) => {
        return core_1.Task.where(`#actor check change order general information`, core_1.Wait.for(core_1.Duration.ofSeconds(5)), assertions_1.Ensure.eventually(web_1.Text.of(components_1.CO.readOnlyFieldLink('Primary Project')), (0, assertions_1.includes)(changeOrderInfo.rowsHash().PrimaryProject)), components_1.CO.checkReadOnlyFieldLinkValue('Primary Contract', changeOrderInfo.rowsHash().PrimaryContract, DefaultStaticParams_1.SUCCEEDED), components_1.CO.checkReadOnlyFieldLinkValue('Contractor', changeOrderInfo.rowsHash().Contractor, DefaultStaticParams_1.SUCCEEDED), assertions_1.Ensure.eventually(web_1.Attribute.called('value').of(components_1.CO.dateInputField('Issue Date')), (0, assertions_1.equals)(changeOrderInfo.rowsHash().IssueDate)), assertions_1.Ensure.eventually(web_1.Attribute.called('value').of(components_1.CO.textInputField('Subject')), (0, assertions_1.equals)(web_1.Cookie.called(statics_1.COOKIE_CO_SUBJECT_NAME).value())));
    }
};
