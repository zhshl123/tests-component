...
exports.addIR = {
    using: (reportName, inspectionType, inspectors, reportStatus, plannedInspectionDate) => {
        const Date = plannedInspectionDate.split(' ')[0];
        const Time = plannedInspectionDate.split(' ')[1] + ' ' + plannedInspectionDate.split(' ')[2];
        return core_1.Task.where(`#actor Input all fields and save `, EditIR_1.IR.fillTextInputField('Report Name', reportName), EditIR_1.IR.selectDropdownItem('Inspection Type', inspectionType), core_1.Wait.for(core_1.Duration.ofSeconds(3)), EditIR_1.IR.selectItemInlookupPopup('Inspectors', inspectors, 'Resource ID'), EditIR_1.IR.selectDropdownItem('Report Status', reportStatus), EditIR_1.IR.selectSpecialDate('Planned Inspection Date', Date, 0), EditIR_1.IR.selectClock('Planned Inspection Date', Time), common_1.clickButton.using(DefaultStaticParams_1.SAVE), (0, common_1.waitMessagePopupBoxVisible)());
    }
};
exports.deleteIR = {
    using: () => {
