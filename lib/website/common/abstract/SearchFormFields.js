...
class SearchFromFields {
    constructor(entityMap) {
        ...
        */
         ...
         * @param itemName
         */
        this.searchItemAndEdit = (pageName, fieldName, itemName) => core_1.Task.where(`#actor check search item: ${itemName} and forward to edit page`, this.searchItemInBrowsePage(pageName, fieldName, itemName), this.checkSearchResult(itemName, DefaultStaticParams_1.SUCCEEDED), ClickButton_1.clickButtonInList.using(DefaultStaticParams_1.EDIT));
        /**
        * 在lookup Popup弹窗中搜索并选择目标选项（当为多选时，全选，当为单选时，默认第一个选项）
        * @param fieldName 字段名称
        * @param itemName 要搜索的关键词
