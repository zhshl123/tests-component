...
exports.addBulletin = {
    using: (BulletinInfo) => {
        return core_1.Task.where(`#actor Input all fields and save `, EditBulletin_1.bulletin.setCookie(statics_1.COOKIE_BULLETIN_ID, web_1.Attribute.called('value').of(EditBulletin_1.bulletin.textInputField('Bulletin No.'))), EditBulletin_1.bulletin.selectItemInlookupPopup('Primary Project', BulletinInfo.rowsHash().PrimaryProject, 'Project Name'), EditBulletin_1.bulletin.selectItemInlookupPopup('Primary Contract', BulletinInfo.rowsHash().PrimaryContract, 'Contract Name'), EditBulletin_1.bulletin.fillTextInputField('Bulletin Title', BulletinInfo.rowsHash().BulletinTitle), common_1.clickButton.using(DefaultStaticParams_1.SAVE), (0, common_1.waitMessagePopupBoxVisible)(), core_1.Wait.for(core_1.Duration.ofSeconds(2)));
    }
};
