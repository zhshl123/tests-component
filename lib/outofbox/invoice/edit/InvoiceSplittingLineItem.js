...
const fillFields = {
    using: (item) => core_1.Task.where(`#actor fill invoice splitting line item fileds`, components_1.invoiceLineitem.selectLookupDropdownItem(item.rowNumber, 'Project', item.Project), components_1.invoiceLineitem.selectLookupDropdownItem(item.rowNumber, 'Fund', item.Fund), components_1.invoiceLineitem.fillNumberInputField(item.rowNumber, 'Percentage', item.Percentage), components_1.invoiceLineitem.fillNumberInputField(item.rowNumber, 'Amount', item.Amount))
};
