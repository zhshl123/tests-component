...
exports.checkCSPInfo = {
    using: (CSPInfo) => {
        return core_1.Task.where(`#actor checks safety plan information`, components_1.csp.checkDropdownInputFieldValue('Category', CSPInfo.rowsHash().Category, DefaultStaticParams_1.SUCCEEDED), assertions_1.Ensure.eventually(web_2.Text.of(components_1.csp.lookupInputFieldSingleValue('Primary Project')), (0, assertions_1.includes)(CSPInfo.rowsHash().PrimaryProject)), assertions_1.Ensure.eventually(web_2.Text.of(components_1.csp.lookupInputFieldSingleValue('Primary Contract')), (0, assertions_1.includes)(CSPInfo.rowsHash().PrimaryContract)), components_1.csp.checkTextInputFieldValue('ID', web_1.Cookie.called(statics_1.COOKIE_CSP_ID).value(), DefaultStaticParams_1.SUCCEEDED), components_1.csp.checkTextInputFieldValue('Title', web_1.Cookie.called(statics_1.COOKIE_CSP_TITLE).value(), DefaultStaticParams_1.SUCCEEDED), components_1.csp.checkDateInputFieldValue('Date Reported', CSPInfo.rowsHash().DateReported, DefaultStaticParams_1.SUCCEEDED));
    }
};
const deleteCSP = () => {
