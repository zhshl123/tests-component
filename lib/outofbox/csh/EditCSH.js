...
exports.checkCSHInfo = {
    using: (CSHInfo) => {
        return core_1.Task.where(`#actor checks construction schedule information`, components_1.csh.checkDropdownInputFieldValue('Category', CSHInfo.rowsHash().Category, DefaultStaticParams_1.SUCCEEDED), assertions_1.Ensure.eventually(web_1.Text.of(components_1.csh.lookupInputFieldSingleValue('Primary Project')), (0, assertions_1.includes)(CSHInfo.rowsHash().PrimaryProject)), assertions_1.Ensure.eventually(web_1.Text.of(components_1.csh.lookupInputFieldSingleValue('Primary Contract')), (0, assertions_1.includes)(CSHInfo.rowsHash().PrimaryContract)), components_1.csh.checkTextInputFieldValue('ID', web_1.Cookie.called(statics_1.COOKIE_CSH_ID).value(), DefaultStaticParams_1.SUCCEEDED), components_1.csh.checkTextInputFieldValue('Title', web_1.Cookie.called(statics_1.COOKIE_CSH_TITLE).value(), DefaultStaticParams_1.SUCCEEDED), components_1.csh.checkDateInputFieldValue('Date Reported', CSHInfo.rowsHash().DateReported, DefaultStaticParams_1.SUCCEEDED));
    }
};
const deleteCSH = () => {
