...
exports.updateDocumentInfo = {
    using: (documentInfo) => {
        ...
        return core_1.Task.where(`#actor update document information`, EditDocumentFields_1.editDocument.fillTextInputField('Document Name', documentInfo.rowsHash().DocumentName + timestamp), EditDocumentFields_1.editDocument.setCookie(statics_1.COOKIE_DOCUMENT_NAME, documentInfo.rowsHash().DocumentName + timestamp), EditDocumentFields_1.editDocument.selectDropdownItem('Status', documentInfo.rowsHash().Status), EditDocumentFields_1.editDocument.fillTextInputField('Key Words', documentInfo.rowsHash().KeyWords), EditDocumentFields_1.editDocument.fillTextInputField('Description', documentInfo.rowsHash().Description), common_1.clickButton.using(DefaultStaticParams_1.SAVE), core_1.Wait.for(core_1.Duration.ofSeconds(5)));
    }
};
